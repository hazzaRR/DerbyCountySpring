...
public interface FixtureRepository extends JpaRepository<Fixture, Integer> {
    ...
    @Query(value = "SELECT * FROM upcoming_fixtures WHERE kickoff LIKE %?1% AND competition = ?2 AND stadium = ?3 AND team = ?4",
            nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM upcoming_fixtures WHERE kickoff LIKE %?1% OR away_team ?2 OR home_team = ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM upcoming_fixtures WHERE kickoff LIKE %?1% AND competition = ?2 OR away_team ?3 OR home_team = ?3", nativeQuery = true)
    List<Fixture> findFixturesByYearAndCompetitionAndTeam(String year, String competition, String team);


    @Query(value = "SELECT * FROM upcoming_fixtures WHERE kickoff LIKE %?1% AND stadium = ?2 OR away_team ?3 OR home_team = ?3", nativeQuery = true)
